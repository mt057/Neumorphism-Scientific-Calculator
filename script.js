...
// Get the sinh button
const sinhButton = document.getElementById('sinh');

// Function to calculate sinh
function sinh(x) {
  return (Math.exp(x) - Math.exp(-x)) / 2;
}

// Add click event listener to sinh button
sinhButton.addEventListener('click', () => {
  // Get the current value from the display
  const currentValue = parseFloat(display.value);
  
  // Calculate sinh of the current value and update the display
  display.value = sinh(currentValue).toFixed(6);
});
...
(function(window) {
	...
	var calcSS3 = document.querySelector('.calculator'),
		// display things
